...
# 填入一次，包含该词的关键词都会统计, 自己填入可能的单复数单词
wanted_words = "dog dogs toy toys"
wanted_words = wanted_words.lower()
print("[INPUT]", wanted_words)
...
for keyword in keyword_rank_list[2:]:
    for wanted_word in wanted_words.split():
...
output_wc_csv_file = "_".join(wanted_words.split()) + "_word_count_" + t + ".csv"
...
# 把关键词和排名导出为csv
# keyword, rank
output_kr_csv_file = "_".join(wanted_words.split()) + "_keyword_rank_" + t + ".csv"
result_csv_folder = "result"
